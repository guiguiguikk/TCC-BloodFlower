...
$sql = "SELECT a.*, u.nome AS usuario_nome, p.nome AS produto_nome FROM avaliacoes a JOIN usuarios u ON a.usuario_id = u.id_usuario JOIN produtos p ON a.produto_id = p.id WHERE id_avaliacao = $id";
...
<body>
    <div class="produto-box">
        ...
        <form action="updateAvaliacao.php" method="POST" enctype="multipart/form-data">
            <input type="hidden" name="id_avaliacao" value="<?= $avaliacao['id_avaliacao'] ?>">
            <div class="mb-3">
                <label for="nome" class="form-label">Comentario</label>
                <input type="text" class="form-control" id="nome" name="comentario" placeholder="Digite o comentario" value="<?= htmlspecialchars($avaliacao['comentario']) ?>" required>
            </div>

            <div class="mb-3">
                <label for="preco" class="form-label">Nota</label>
                <input type="number" class="form-control" id="preco" name="nota" placeholder="Nota de 1 a 5" value="<?= htmlspecialchars($avaliacao['nota']) ?>" required min="1" max="5">
            </div>
            ...
            <div class="mb-3">
                ...
                <select class="form-select" id="categoria" name="usuario" required>
                    <option value="<?= htmlspecialchars($avaliacao['usuario_id']) ?>" selected><?= htmlspecialchars($avaliacao['usuario_nome']) ?></option>
            ...
            <div class="mb-4">
                ...
                <select class="form-select" id="marca" name="produto" required>
                    <option value="<?= htmlspecialchars($avaliacao['produto_id']) ?>" selected><?= htmlspecialchars($avaliacao['produto_nome']) ?></option>
